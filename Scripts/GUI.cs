...
//the current version i am using is below, u can take a look and combine them together~~~~~
//thx!!
/*
using UnityEngine;
using System.Collections;
...
public class IO: MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if(GUI.Button (new Rect (0, 0, 5,Screen.height), "")){
		...
		if(menubar_out){
			...
			if (inventorybool == true) {
				GUI.Box (new Rect (Screen.width / 2 - 300, Screen.height / 2 - 250, 600, 500), "Inventory", inventory);
			}
			...
			//Map button implementation
			if (GUI.Button (new Rect (10, 2*Screen.height/7, Screen.width/14,Screen.height/11), "", mapButton)) {
				mapbool = true;
				mappushcount += 1;
				if (mappushcount % 2 == 0) {
					mapbool = false;
			...
			//End of map button implementation*************************************************************************************
			GUI.Button (new Rect (10, 3*Screen.height/7, Screen.width/14,Screen.height/11), "", promotionButton);
